...
| S2 | `TestS2CMDBObjectChurn` (+probe) | CMDB objects/links churn; probe: object-type cache orphaned by a partial delete | churn PASS, **probe RED (L3)** |
| S3 | `TestS3TypeCascadeChurn` | fresh type + objects + `type.delete` cascade per cycle | PASS |
| S4 | `TestS4FunctionContexts` | (a) TTL context reclaimed; (b) namegen build-error path; (c) context of a deleted object | (a) PASS, **(b) RED (L2)**, **(c) RED (L1)** |
...
| S8 | `TestS8KeyMutex` (+probe) | key-mutex churn; probe: dotted vertex ids | churn PASS, **probe RED (L6)** |
| S9 | `TestS9CacheStore` (+2 probes) | deep-key tombstone sweep; probes: `force=true` link retarget, orphaned `out.to` | sweep PASS, **probes RED (L4, L5)** |
...
| S12 | `TestS12KVGrowthReport` | NATS-side KV/stream growth under fresh-id churn | REPORT only |

## Expected-red checks (known findings, stay red until fixed)

The red assertions state the DESIRED behavior. Fixing a finding flips its
check to green with no test edit. Anything red that is **not** in this table
is a NEW leak.

| Finding | Check | What leaks | Where |
|---|---|---|---|
| **L1** | `s4c_ctx_orphans / orphan_ctx_keys` | function context `<typename>.<id>` written without a TTL survives the object's deletion forever — no cleanup hook on vertex/object delete, GC reclaims only TTL-marked contexts | `statefun/function_type.go` (context set/gc), `embedded/graph/crud` delete paths |
| **L2** | `s4b_namegen_builderr / namegen_ctx_no_ttl` | `triggerfunc.ObjectNameGenerator` stores object+type bodies as its context and only sets the TTL at the end; the executor build-error path returns in between — one permanent context per touched object | `embedded/graph/triggerfunc/triggerfunc.go` (SetFunctionContext → BuildError return → SetContextExpirationAfter) |
| **L3** | `s2_otc_orphan / object_type_cache` | a partially deleted object (body key gone → `vertex.delete` replies IDLE) leaves its entry in the process-global `objectTypeCache` forever | `embedded/graph/crud/hl_crud_helpers.go` (objectTypeCache), `ll_crud.go` (idle short-circuit) |
| **L4** | `s9_force_retarget / cache_live_values` | `link.create force=true` with a new target strands the old target's `ltype`/`in` keys; no delete path ever removes them | `embedded/graph/crud/ll_crud.go` (force branch vs `writeOutLinkKV`) |
| **L5** | `s9_orphan_outto / cache_live_values` | a link missing its `out.to` key is undeletable: `link.delete` and `vertex.delete` reply IDLE and leave `out.body`/`out.index`/`ltype`/`in` keys forever; no repair path | `embedded/graph/crud/ll_crud.go` (target resolution in delete paths) |
| **L6** | `s8_dotted_id / graph_keymutex_entries, cache_active_ops, cache_pending_txs` | a dotted vertex id breaks `operationKeysMutexUnlock` payload parsing: the KeyMutex entry stays permanently write-locked AND `MarkOperationDone` is skipped — the orphaned `activeOps` entry wedges the WAL publisher for the runtime's lifetime | `embedded/graph/crud/ll_crud.go` (`__key_locks` parsing; no id validation) |
