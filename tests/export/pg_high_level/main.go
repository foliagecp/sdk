// Foliage export test package — semantic (high-level) PostgreSQL dumper.
// Tests the full export pipeline with CMDB-level classification:
// CMDB CRUD → WAL → ExportCommitter → SemanticTranslator → SemanticPGDumper → PostgreSQL.
package main
...
func main() {
	...
	if *helpFlag {
		fmt.Println("usage: foliage-semantic-export-test [option]")
		fmt.Println("Options:")
		flag.PrintDefaults()
		return
	}
