...
func FieldValuePartialMatch(_ sfPlugins.StatefunExecutor, ctx *sfPlugins.StatefunContextProcessor) {
	...
	for _, domain := range ctx.Domain.GetWeakClusterDomains() {
		objectIds, err := dbc.Query.JPGQLCtraQuery(domain+statefun.ObjectIDDomainSeparator+crud.BUILT_IN_OBJECTS, fmt.Sprintf(".*[l:type('%s')]", crud.OBJECTS_OBJECT_TYPELINK))
		if err != nil {
